...
class DireccionEnvioForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'direccion': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Ingrese su dirección exacta'}),
        }

    region = forms.ChoiceField(
        choices=DireccionEnvio.REGION_CHOICES,
        widget=forms.Select(attrs={'class': 'form-control'}),
        initial=DireccionEnvio.REGION_METROPOLITANA,
        label="Región"
    )

    comuna = forms.ChoiceField(
        choices=DireccionEnvio.COMUNA_CHOICES,
        widget=forms.Select(attrs={'class': 'form-control'}),
        label="Comuna"
    )
